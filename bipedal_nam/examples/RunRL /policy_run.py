...
"""
- laptop chạy policy run, kết nối với 2 server mb1,2 qua zmq để điều khiển (gửi lệnh xuống)
- loop 20hz, làm các việc 
...
# Chuyển imu data thành data để match với số trong training
def scale_imu(orient_raw, gyro_raw, orient_min=-1.0, orient_max=1.0, gyro_min=-2.0, gyro_max=2.0):
    """
    Args:
    ...
    for val in orient_raw[:2]:
        ...
        if orient_max == orient_min:
            ...
            normalized = (clipped - orient_min) / (
                orient_max - orient_min
            ) * 2.0 - 1.0  # công thức scaling chính của orientation
    ...
    for val in gyro_raw[:3]:
        ...
        if gyro_max == gyro_min:
            ...
            normalized = (clipped - gyro_min) / (
                gyro_max - gyro_min
            ) * 2.0 - 1.0  # công thức scaling của tốc độ góc từ gyro
...
# Phần sắp xếp lại thành 44D như trong sim
...
# Neural network output ra 6 số action, cần convert thành servo angles (degrees)
def process_actions(robot, model_actions, last_actions):
    ...
    actions = np.clip(model_actions, -3.0, 3.0)  # giới hạn lại số từ actions

    # Delta accumulation
    # mỗi bước tối đa nhích 3 độ, cộng dồng vào last action
    actions = np.array(last_actions) + (actions * 4 / 3)
    # Clip to joint limits (6 joints)
    servo_min = [20, 20, -55, -55, 20, 20]  # kẹp góc khớp (độ)
...
# chuyển ngược lại góc khớp (độ) sang dải -1 1 của action
...
def update_buffers(
    robot, state_orient, state_gyro, last_actions, orient_history, gyro_history, action_history
):
    """
...
def perform_inference(policy_model, observations):
    ...
    if isinstance(policy_output, torch.Tensor):
        ...
        policy_actions = (
            policy_output[0] if isinstance(policy_output, (list, tuple)) else policy_output
        )
...
def main():
    ...
    robot = TransformerAPI(
        left_host="mobile2.local", left_port=5556, right_host="mobile1.local", right_port=5555
    )
    ...
    while not imu_ready and (time.time() - wait_start) < max_wait:
        try:
            ...
            if not (
                abs(orient[0]) < 0.001
                and abs(orient[1]) < 0.001
                and abs(gyro[0]) < 0.001
                and abs(gyro[1]) < 0.001
            ):
                imu_ready = True
                print(f"✅ IMU ready!")
                break
