...
# PUB/SUB: port phat IMU = zmq_port + offset nay (5555 -> 5755, 5556 -> 5756).
# Phai TRUNG voi IMU_PUB_PORT_OFFSET trong bipedal_robot/sensors/imu_pubsub.py.
...
class MCUServerLeft:
    ...
    def __init__(self, zmq_port: int = 5556, serial_port: str = "/dev/ttyACM0"):
        ...
        # PUB/SUB: so thu tu mau IMU da phat. Client dem so nay de biet co
        # mau nao roi tren duong mang khong (seq nhay coc = roi).
        self.imu_seq = 0
    ...
    def _publish_imu(self, pub: "zmq.Socket") -> None:
        """Phat mau IMU vua xu ly xong len socket PUB (khong chan).

        Noi dung GIONG HET phan IMU trong reply "feedback" cua REQ/REP, de
        client doc duong nao cung ra cung mot so. Them "seq" de client dem
        mau roi.
        """
        with self.imu_lock:
            msg = {
                "seq": self.imu_seq,
                "quat": self.state_data["imu"].copy(),
                "gyro": self.state_data["gyro_rad"].copy(),  # rad/s, da tru bias
                "t_sample": self.state_data["imu_t_sample"],
            }
        self.imu_seq += 1
        try:
            # NOBLOCK: neu hang doi gui day (laptop tat / mang nghen) thi BO mau
            # nay chu khong dung imu_loop lai - vong fusion 50Hz quan trong hon.
            pub.send_json(msg, zmq.NOBLOCK)
        except zmq.Again:
            pass

    def imu_loop(self) -> None:
        """✅ IMU thread - 50Hz independent loop + PUB mau IMU ra mang"""
        logger.info("IMU loop started (50Hz - independent)")

        # Socket PUB phai tao TRONG thread nay: socket ZMQ khong duoc dung cheo
        # thread (context thi dung chung duoc). Port = zmq_port + 200, cung quy
        # tac voi PULL (+100). SNDHWM nho de khong tich hang nghin mau trong
        # RAM khi khong ai nghe.
