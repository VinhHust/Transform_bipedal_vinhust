...
# API NÀY DÙNG ĐỂ ĐIỀU KHIỂN ĐỘNG CƠ QUA BẰNG 2 SERVER
...
# Configure logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(levelname)s - [TRANSFORMER] - %(message)s"
)
...
class TransformerAPI:
    ...
    def __init__(
        ...
        self.servo_config_left = {
            5: {"name": "hip", "home_ticks": 2044, "min_ticks": 1676, "max_ticks": 2396},
            7: {"name": "knee", "home_ticks": 2020, "min_ticks": 996, "max_ticks": 3051},
            8: {"name": "foot", "home_ticks": 2117, "min_ticks": 1416, "max_ticks": 2711},
        }
    ...
    def initialize(self, warmup_time: float = 5.0) -> bool:
        ...
        try:
            ...
            if self.imu_fusion is not None:
                ...
                logger.warning(
                    "⚠️  IMU fusion chưa khởi tạo - bỏ qua thread IMU, orient/gyro sẽ luôn = 0"
                )
    ...
    def degree_to_ticks(self, leg: str, servo_id: int, degrees_relative: float) -> int:
        ...
        # CHIỀU PHẢI CONFIRM LẠI CÁI NÀY
        if leg == "right":
    ...
    try:
        ...
        for cycle in range(num_cycles):
            ...
            for step in range(steps_per_cycle):
                ...
                # Apply joint angles
                success = robot.set_joint_angles(hip_l, knee_l, foot_l, hip_r, knee_r, foot_r)
