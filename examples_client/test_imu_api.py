...
def main():
    ...
    try:
        ...
        print("  Terminal 1: python -m src.leg_server.leg_server_left --port 5556 --debug")
        print("  Terminal 2: python -m src.leg_server.leg_server_right --port 5555 --debug")
        return
    ...
    try:
        while True:
            ...
            yaw_deg *= 180 / math.pi
            print(f"  FUSED: Roll={roll_deg:+7.2f}°  Pitch={pitch_deg:+7.2f}°  Yaw={yaw_deg:+7.2f}°")
            
            # ✅ THÊM: Display GYRO data
            print("\n🌀 GYRO (Angular Velocity) - Baselink Frame [rad/s]:")
            print(f"  LEFT:  GX={left_gyro[0]:+.6f}  GY={left_gyro[1]:+.6f}  GZ={left_gyro[2]:+.6f}")
            print(f"  RIGHT: GX={right_gyro[0]:+.6f}  GY={right_gyro[1]:+.6f}  GZ={right_gyro[2]:+.6f}")
            print(f"  FUSED: GX={fused_gyro[0]:+.6f}  GY={fused_gyro[1]:+.6f}  GZ={fused_gyro[2]:+.6f}")
            ...
            # Display timestamp
            print(f"\n⏱️  Timestamp: {timestamp:.3f}")
            
            # Display data in compact format (useful for logging)
            print("\n📋 COMPACT FORMAT (for logging/analysis):")
            print(f"  Fused Quat: [{fused_quat[0]:+.4f}, {fused_quat[1]:+.4f}, {fused_quat[2]:+.4f}, {fused_quat[3]:+.4f}]")
            print(f"  Fused Euler (deg): [{roll_deg:+7.2f}°, {pitch_deg:+7.2f}°, {yaw_deg:+7.2f}°]")
            print(f"  Fused Gyro (rad/s): [{fused_gyro[0]:+.6f}, {fused_gyro[1]:+.6f}, {fused_gyro[2]:+.6f}]")
