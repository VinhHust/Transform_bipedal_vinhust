...
IMU.setFullScaleRangeGyro(qwiic_icm20948.dps500)  # cùng FSR với lúc calib
IMU.setFullScaleRangeAccel(qwiic_icm20948.gpm4)  # cùng FSR với lúc calib accel
...
try:
    ...
    while True:
        if IMU.dataReady():
            ...
            print(f"\n[RAW DATA]")
            print(
                f"  Accelerometer (raw):  ax={ax_raw:6.0f}, ay={ay_raw:6.0f}, az={az_raw:6.0f}"
            )
            print(
            ...
            print(
                f"  Gyroscope (°/s):      gx={gx_deg:+.2f}, gy={gy_deg:+.2f}, gz={gz_deg:+.2f}"
            )
            print(f"  Gyroscope (rad/s):    gx={gx:+.4f}, gy={gy:+.4f}, gz={gz:+.4f}")

            print(f"\n[MADGWICK QUATERNION]")
            print(
                f"  q = [{q_new[0]:+.6f}, {q_new[1]:+.6f}, {q_new[2]:+.6f}, {q_new[3]:+.6f}]"
            )
            print(f"  (w, x, y, z)")
